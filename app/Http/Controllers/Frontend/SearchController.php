...
class SearchController extends Controller
{
    public function search(Request $request)
    {
        ...
        $brandsBulder = Brand::where(['is_active' => Brand::ACTIVE, 'brands.locale_id' => $domain->id]);
        if ($search) {
            $brandsBulder->where('brands.showcase', 'LIKE', "%$search%")
                   ->orWhere('brands.name', 'LIKE', "%$search%")
                   ->where(['brands.is_active' => Brand::ACTIVE, 'brands.locale_id' => $domain->id])
        ...
        ->where(['pages.status' => Page::ACTIVE, 'pages.locale_id' => $domain->id])
        ...
        $pagesBulder = Page::where(['status' => Page::ACTIVE, 'locale_id' => $domain->id, 'type' => Page::DEFAULT_PAGE]);

        if ($search) {
            $pagesBulder->where('add_content', 'LIKE', "%$search%")
                   ->orWhere('title', 'LIKE', "%$search%")
                   ->where(['status' => Page::ACTIVE, 'locale_id' => $domain->id, 'type' => Page::DEFAULT_PAGE])
