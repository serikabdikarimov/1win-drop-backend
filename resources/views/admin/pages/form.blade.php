            ...
            @elseif(key((array)$blocks) == 'login')
                <div class="form-group" data-block-index="{{ $index }}" data-block-name="{{ 'login' }}">
                    ...
                    <div class="login-form">
                        <div class="row">
                            <div class="col-md-6">
                                <input type="text" name="add_content[{{ $index }}][login][play][name]" class="form-control" placeholder="Введите наименование кнопки войти" value="{{ $blocks->login->play->name }}" required>
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="add_content[{{ $index }}][login][play][url]"  class="form-control" placeholder="Введите ссылку входа" value="{{ $blocks->login->play->url }}" required>
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="add_content[{{ $index }}][login][register][name]" class="form-control" placeholder="Введите наименование кнопки регистрации" value="{{ $blocks->login->register->name }}" required>
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="add_content[{{ $index }}][login][register][url]"  class="form-control" placeholder="Введите ссылку регистрации" value="{{ $blocks->login->register->url }}" required>
                            </div>
                        </div>
                    </div>
                </div>
