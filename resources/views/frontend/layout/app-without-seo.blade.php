...
<head>
    ...
    <title>{{ __('messages.Личный кабинет') }}</title>
...
<body>
    ...
    <!-- Modal Avatar Picker Begin -->
    <div class="modal modal--sizeLarge modal--slide" id="modal-avatar-picker" aria-hidden="true">
        <div class="modal__dialog" role="dialog" aria-labelledby="">
            <div class="modal__header text--center">
                <h2 class="modal__headerTitle">
                {{__('messages.Выберите аватар')}}
            </h2>
                <button type="button" class="modal__closeButton" data-micromodal-close aria-label="Close">
                    <span class="icon icon--colorBase icon--applyColor">
                    <svg viewBox="0 0 20 20" class="icon__svg" focusable="false" aria-hidden="true"><path d="m11.414 10 6.293-6.293a1 1 0 1 0-1.414-1.414l-6.293 6.293-6.293-6.293a1 1 0 0 0-1.414 1.414l6.293 6.293-6.293 6.293a1 1 0 1 0 1.414 1.414l6.293-6.293 6.293 6.293a.998.998 0 0 0 1.707-.707.999.999 0 0 0-.293-.707l-6.293-6.293z"></path></svg>
                </span>
                </button>
            </div>
            <form action="/update-avatar" method="POST">
                {{ csrf_field() }}
                <div class="modal__bodyWrapper">
                    <div class="modal__body scrollable scrollable--vertical" data-scrollable="true">
                        <div class="modal__section">
                            <fieldset>
                                <ul class="avatar-picker">
                                    @foreach ($avatarsCategory->images as $image)
                                        <li class="avatar-picker__item">
                                            <label class="avatar-picker__choice @if($loop->iteration == 1) actionList__item--selected @endif" for="avatar{{$image->id}}">
                                                <input @if($loop->iteration == 1) checked @endif id="avatar{{$image->id}}" name="avatars" type="radio" class="avatar-picker__input" value="{{$image->id}}">
                                                <img src="/storage/uploads/{{ $image->url }}" class="avatar-picker__img" title="" loading="lazy" alt=""> 
                                            </label>
                                        </li> 
                                    @endforeach
